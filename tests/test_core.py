...
def test_convert_value():
    ...
    assert _convert_value("invalid", "num") == float("-inf")
    ...
    # Empty values
    assert _convert_value("", "num") == float("-inf")
...
def test_sort_iter_multi_key():
    ...
    actual_names = [item["name"] for item in sorted_data]
...
def test_sort_iter_natural_sorting():
    ...
    # Regular string sort would give: file1, file10, file2, file20
    # Natural sort should give: file1, file2, file10, file20
    sorted_data = list(sort_iter(data, keys=[key("filename", "nat")]))
    
    filenames = [item["filename"] for item in sorted_data]
    assert filenames == ["file1.txt", "file2.txt", "file10.txt", "file20.txt"]
